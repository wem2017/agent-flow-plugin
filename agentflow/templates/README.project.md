...
Đây là terminal mode — mặc định thì break-out của orchestrator **chính là** notification; theo dõi session này để bắt chúng. (Bạn có thể chạy `/start` không giám sát theo một interval qua skill `/loop`; khi đó các break-out sẽ queue trên board cho tới khi bạn quay lại.)

**Tùy chọn — ping Telegram.** Bật `connections.notify.enabled: true` trong `.claude/agentflow.yaml` và điền `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` vào `.env` để `/start` nhắn cho bạn mỗi khi pipeline dừng lại chờ người: ticket rơi vào `Refined`, DEV báo blocked, một run bị stuck, hoặc một PR đã sẵn sàng merge. Chỉnh `connections.notify.events` để bớt ồn. Đây là ping **một chiều cho bạn** — agent không đọc kênh này và vẫn chỉ phối hợp qua board. Chưa điền secret thì notify tự tắt, mọi thứ khác chạy như thường.
