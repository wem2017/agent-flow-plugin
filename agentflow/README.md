...
Role boundary là behavioral contract trong prompt từng agent — PMO không sửa code, chỉ DEV mở branch/PR và push feature code, QC chỉ đụng test file/test ID trên PR branch. Từ **v0.7.0**, một phần của boundary đó được **enforce thật ở tầng tool** qua `disallowedTools` trong frontmatter mỗi agent (không còn là "khi được cấu hình"): cả ba agent **mất hẳn** `merge_pull_request` — human merge gate giờ không phải honor-system; PMO thêm nữa mất `create_pull_request` / `pull_request_review_write` / `Edit` / `Write`; DEV mất `pull_request_review_write` (không tự approve PR của mình); QC mất `create_pull_request` (chỉ push lên PR branch sẵn có). Xem "Ghi chú & giới hạn" để biết chính xác phần nào **chưa** được enforce. Cả ba agent load được core skill và role-prefixed skill của chúng, và mỗi agent tự thực hiện Status transition của chính nó theo write order của protocol. Toàn bộ wire protocol — comment prefix, DoR/DoD, state section trong issue body, và trust rule — là skill `project-board-protocol`.
...
- **Safety rule: một phần đã enforce, phần còn lại vẫn ở mức prompt — đừng nhầm hai cái.**
  - **Đã enforce (tầng tool, v0.7.0+):** `disallowedTools` trong frontmatter mỗi agent gỡ hẳn `merge_pull_request` khỏi cả ba agent, cộng các restriction theo role (xem "Pipeline hoạt động ra sao"). Một agent bị derail/inject **không gọi được** MCP tool merge.
...
- **Role isolation = prompt contract + `disallowedTools` đã ship** — đừng nới capability của role nào "để phòng hờ" (cả ba agent dùng chung một token, nên blast radius là thật). Cụ thể: **đừng bao giờ trả `merge_pull_request` lại cho một sub-agent** — human merge gate là non-goal cứng, và giờ nó được enforce ở tầng tool chứ không chỉ ở prose.
