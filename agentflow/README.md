...
Một Claude Code plugin biến một coding agent đơn lẻ thành một team nhỏ, có trách nhiệm rõ ràng. Bốn agent — **PMO (Product Owner + Product Manager)**, **Designer (DESIGNER)**, **Developer (DEV)**, và **Quality Control (QC)** — phối hợp trên GitHub repo của bạn, để bạn chỉ phải làm hai việc bằng tay: **mô tả công việc** và **review/merge cái PR cuối cùng**.

DESIGNER là **có điều kiện**: chỉ ticket đụng tới một surface được đánh dấu `ui: true` mới đi qua bước design. Repo backend-only chạy y hệt như khi chưa có nó.

AgentFlow **tech-stack-agnostic**. Bạn cài plugin **một lần** và chạy nó theo từng repo; mỗi repo có `.claude/agentflow.yaml` riêng — single source of truth mô tả connections, secrets, surfaces, skills, labels, và rule của agent. Các agent và năm core skill **chỉ** đọc file này. Điểm nổi bật của **v0.8.0**:

- **Năm core skill tập trung** — `setup-agentflow`, `project-board-protocol`, `git-flow-working`, `figma-design`, `design-system-rules`. Hướng dẫn onboarding/setup, wire protocol, git flow, Figma handoff, và design system rules đi kèm plugin và load on demand.
- **Project skill mở rộng theo prefix của role** — thả skill của riêng bạn vào `.claude/skills/` đặt tên `dev-*` / `qc-*` / `pmo-*` / `designer-*`; agent tương ứng sẽ nhận chúng. Đăng ký chúng trong `skills:` để có cái nhìn tổng quan, hoặc dựa vào auto-discovery theo prefix.
...
- **Single source of truth** — `.claude/agentflow.yaml` mô tả connections, secrets, surfaces, skills, labels, và rule của agent. Các agent và năm core skill **chỉ** đọc file này.
...
- **`Status` field trên Projects v2 board** — state authoritative (việc cần làm tiếp theo); happy path `Inbox → [In Design] → Ready for Dev → In Progress → In QC → Ready for Human Review → Done`, trong đó `In Design` chỉ xuất hiện với ticket UI (các lane design gate / rework / escalation / human PR-feedback: xem "Pipeline hoạt động ra sao").
...
GitHub Projects v2 board là **bắt buộc** — Status field của nó mang state, và board đồng thời là inbox queue của orchestrator. **Label không mang state**: label chỉ còn classification `type/*`, `component/<surface>`, và hai aux `rework` + `design-review`. Mọi agent (PMO/DESIGNER/DEV/QC lẫn orchestrator) đọc và ghi state qua các tool `projects_*` — mỗi agent tự thực hiện transition của chính nó. Figma là **tùy chọn** — DESIGNER dùng để dựng frame và DEV dùng để đọc spec trong lúc làm UI, khi connection `figma` được enable.
...
| `TELEGRAM_BOT_TOKEN` | Không | connection `notify` | Bot token từ `@BotFather`. Chỉ cần khi bật notify. |
| `TELEGRAM_CHAT_ID` | Không | connection `notify` | Chat/group id nhận ping (`@userinfobot`; group id thường âm). Chỉ cần khi bật notify. |
...
| `notify` | Ping **một chiều tới con người** khi `/start` break-out (Telegram). Cần cho việc chạy `/loop` unattended. Không có MCP server — HTTPS thuần. | `channel`, `auth.token_env`, `target_env`, `events` | Tùy chọn (`enabled: true` + `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID`). Send là **best-effort**, fail không bao giờ dừng pipeline. Xem `/start` → "Notifications". |
...
| `figma-design` | Kéo frame spec/token qua `figma` MCP server (với REST fallback) cho việc handoff từ design sang implementation. Đường **đọc**, phía DEV. |
| `design-system-rules` | Resolve design system rules từ ba nguồn theo thứ tự ưu tiên cố định (rules file trong repo → Figma library → suy ra từ codebase), cộng compliance checklist và script `check-design-compliance.sh`. Reference đi kèm `design-artifacts.md` bao gồm folder convention, format spec, và Figma **write** mode. |
...
| Command | Nó làm gì |
|---------|--------------|
| `/agentflow-init` | Bootstrap một lần **theo từng repo**: cấu hình `connections.*`, xác minh secret `env:`, khai báo các `surfaces.*` bạn có (`path` + `label` + `forbidden_paths`), scaffold/đăng ký project skill, tạo label classification `type/*` + `component/<surface>` + hai aux `rework` + `design-review` (không có label state), tạo/link Projects v2 board **bắt buộc** (Status field 8 option: init tự thêm option còn thiếu sau khi bạn duyệt, fallback thủ công qua UI; built-in workflows vẫn là bước UI thủ công — không API nào config được; init validate lại qua MCP), sinh `.claude/agentflow.yaml` và `README.agentflow.md`, và chạy một verification ticket end-to-end. Chạy lại trên repo v0.3.x để migrate (xem "Migration"). |
...
| `/design [#issue \| description]` | Chạy DESIGNER trên một ticket UI: biến brief của PMO thành design artifact (file trong design folder và/hoặc frame Figma + spec) theo design system rules, rồi handoff cho DEV. DESIGNER tự quyết đây là design **mới** hay **update** bằng cách đọc artifact hiện có — bạn không phải khai báo. |
| `/design-review #issue` | Design-QC trên UI mà DEV đã build: đối chiếu implementation với artifact + rules, rồi ✅ (→ human review) hoặc ❌ (→ DEV kèm `rework`). Khi `design.design_review: true`, QC tự route vào pass này sau khi ✅ và bạn không cần gõ command. |
| `/status` | In số lượng open-issue theo từng Status column cho repo này; `--audit` chạy membership + reconcile + visibility/orphan check; **`--metrics`** tính flow metrics (throughput, cycle time median/p90, first-pass yield, rework & escalation rate, DoR bounce, WIP, và danh sách ticket đang chờ người quá lâu) suy ra từ các transition comment + Status trên board — `--since <N>d` để đổi window (mặc định 30d). |
...
design gate (CÓ ĐIỀU KIỆN — ticket không đụng UI bỏ qua hoàn toàn):
  ...
  In Design ──(brief mơ hồ / rules mâu thuẫn / không có design source)──▶ Refined
...
design review (chỉ khi design.design_review: true) — tái dùng In Design theo chiều ngược:
  In QC ──QC ✅ trên ticket UI──▶ In Design  (+ aux design-review)
       ──[DESIGNER] ✅──▶ Ready for Human Review
       ──[DESIGNER] ❌──▶ Ready for Dev  (+ rework, tính vào cùng ngưỡng escalation 2)
...
- **PMO** biến message của bạn thành một issue chỉn chu ngay tại "Inbox", gắn label `type/*` và `component/<surface>`, gate nó qua một **Definition of Ready**, viết một design brief **`## For DESIGNER`** (chỉ khi ticket đụng UI), một implementation plan **`## For DEV`**, và một focus verification **`## For QC`** vào issue body (planning bằng cách mô tả, không phải bằng cách dispatch — nó không bao giờ assign hay điều khiển các agent khác). DoR pass → Status "In Design" nếu design gate thoả, ngược lại "Ready for Dev"; nếu cần con người bổ sung info thì park sang "Refined" kèm một vòng câu hỏi `[PMO]`. PMO không tự trả lời câu hỏi làm rõ của DEV/QC — mọi info-gap đều để con người xử lý ở "Refined", và PMO re-triage khi ticket quay lại "Inbox". (Không bao giờ viết code hay merge — hard rule.)
- **DESIGNER** (chỉ ticket UI) biến brief của PMO thành design artifact thật — file trong `design.folder` và/hoặc frame Figma — **luôn** resolve design system rules trước theo thứ tự ưu tiên cố định (rules file trong repo → Figma library → suy ra từ codebase), rồi viết một spec mà DEV implement được. Nó tự quyết đây là design **mới** hay **update** bằng cách đọc artifact hiện có, chứ không hỏi. Ở folder mode nó commit artifact lên **chính feature branch mà DEV sẽ dùng** (`agent/dev/<kind>/<issue#>-<slug>`) nên design đi cùng chuyến với code trong đúng một PR; ở Figma-only mode nó không đụng git và spec đi vào section `## Design` của issue body. Khi bật `design.design_review`, nó chạy thêm một lần nữa sau QC ✅ để review UI đã build. (Không viết app source code, không mở PR, không merge — hard rule.)
...
Role boundary là behavioral contract trong prompt từng agent — PMO không sửa code, DESIGNER chỉ ghi vào `design.folder` + rules file, chỉ DEV mở branch/PR và push feature code, QC chỉ đụng test file/test ID trên PR branch. Từ **v0.7.0**, một phần của boundary đó được **enforce thật ở tầng tool** qua `disallowedTools` trong frontmatter mỗi agent (không còn là "khi được cấu hình"): cả ba agent **mất hẳn** `merge_pull_request` — human merge gate giờ không phải honor-system; DESIGNER thì được gate theo hướng ngược lại, bằng một `tools:` **allowlist** không hề chứa `merge_pull_request` hay `create_pull_request`; PMO thêm nữa mất `create_pull_request` / `pull_request_review_write` / `Edit` / `Write`; DEV mất `pull_request_review_write` (không tự approve PR của mình); QC mất `create_pull_request` (chỉ push lên PR branch sẵn có). Xem "Ghi chú & giới hạn" để biết chính xác phần nào **chưa** được enforce. Ranh giới path-scoped của **QC** ("chỉ test file") và của **DESIGNER** ("chỉ `design.folder` + rules file") vẫn là **prose, không phải tool grant** — `Edit`/`Write` là all-or-nothing, không scope được theo path. Với DESIGNER, prose đó được backed bằng một check thật: `check-design-compliance.sh --staged` fail khi có file ngoài phạm vi bị staged, và DESIGNER chạy nó trước mỗi lần push. Cả bốn agent load được core skill và role-prefixed skill của chúng, và mỗi agent tự thực hiện Status transition của chính nó theo write order của protocol. Toàn bộ wire protocol — comment prefix, DoR/DoD, state section trong issue body, và trust rule — là skill `project-board-protocol`.
...
- **`design.*`** — `enabled`, `folder` (`""` = figma-only), `rules_file` (nguồn rules ưu tiên #1), `design_review` (chạy DESIGNER lại sau QC ✅). Đi kèm cờ **`surfaces.<key>.ui`** — nơi khai báo **duy nhất** cho "surface nào là UI"; predicate design gate đọc từ đó. `enabled: false` hoặc không surface nào `ui: true` → bước design không bao giờ kích hoạt.
...
- **`board.number` / `board.columns`** — project number của Projects v2 board (một số không rỗng; board là bắt buộc — MCP projects tools key theo owner + number, không phải node id) và tám tên column của Status field. `board.columns` chính là **state enum authoritative**; các option name là wire value được resolve by-name, nên đổi tên một option trong UI là break routing (init validate qua `list_project_fields`; toolset `projects` không có method ghi field, nên init thêm option còn thiếu qua một carve-out `gh api graphql` **có xác nhận của bạn**, với fallback thủ công qua UI — xem skill `project-board-protocol` → reference §"Carve-out: ghi Status field"). Đi cùng `connections.github_project.enabled` (luôn `true`; init giữ chúng đồng bộ).
...
- **Mặc định synchronous; continuous là opt-in.** Mặc định, chính việc break-out ở terminal *là* notification. Bạn có thể chạy `/start` không cần giám sát theo một interval qua skill `/loop` (cadence thích ứng, không phải busy-loop); các break-out khi đó xếp hàng bền vững trên board (Status được park + comment) cho tới khi bạn quay lại. Xem `/start` → "Continuous mode".
- **Notification ra ngoài là tùy chọn (v0.7.0+).** Bật connection `notify` để `/start` mirror mỗi break-out sang **Telegram** — cần thiết nếu chạy `/loop` unattended, vì nếu không, ticket park ở `Refined` / `Ready for Human Review` mà bạn không hề biết. Đây là ping **một chiều tới con người**: không agent nào đọc kênh đó, nó không mang state, và board vẫn là nơi phối hợp duy nhất — nên non-goal "no message bus" không bị vi phạm. Send là best-effort: fail thì chỉ note một dòng, không bao giờ dừng pipeline.
...
- **Safety rule: một phần đã enforce, phần còn lại vẫn ở mức prompt — đừng nhầm hai cái.**
  - **Đã enforce (tầng tool, v0.7.0+):** `disallowedTools` trong frontmatter mỗi agent gỡ hẳn `merge_pull_request` khỏi cả ba agent PMO/DEV/QC (DESIGNER dùng `tools:` allowlist, vốn không chứa nó), cộng các restriction theo role (xem "Pipeline hoạt động ra sao"). Một agent bị derail/inject **không gọi được** MCP tool merge.
  - **CHƯA enforce (vẫn là prompt contract):** `forbidden_paths`, no-force-push, no-push-to-default-branch, và ranh giới "QC không đụng implementation logic". Quan trọng hơn: **`Bash` vẫn là escape hatch** — DEV/QC bắt buộc phải có `Bash` để chạy git + build/test, nên về lý thuyết vẫn `git push` thẳng lên default branch hoặc `gh pr merge` được. `disallowedTools` đóng **đường MCP**, không đóng đường shell.
...
- **`Status` trên Projects v2 board là state authoritative duy nhất; label không bao giờ mang state.** Đừng bao giờ để một label hay một câu trả lời tường thuật của sub-agent trở thành *nguồn* routing. `In Design` là state có điều kiện nhưng option của nó vẫn **bắt buộc tồn tại** trên board — Status write là fail-stop. Việc đọc lại Status live sau mỗi lần chạy chính là thứ làm cho thiết kế no-message-bus hoạt động.
- **Role isolation = prompt contract + `disallowedTools` đã ship** — đừng nới capability của role nào "để phòng hờ" (cả bốn agent dùng chung một token, nên blast radius là thật). Cụ thể: **đừng bao giờ trả `merge_pull_request` lại cho một sub-agent** — human merge gate là non-goal cứng, và giờ nó được enforce ở tầng tool chứ không chỉ ở prose.
- **Human merge gate là bắt buộc.** Agent dừng ở "Ready for Human Review"; chỉ con người merge — đừng automate merge.
- **Đừng giả vờ phần CHƯA enforce là đã enforce.** `disallowedTools` đóng đường MCP (đã ship), nhưng `forbidden_paths` / force-push / push-to-default vẫn là prompt contract, và `Bash` vẫn là escape hatch — docs nói thẳng như vậy ở "Ghi chú & giới hạn". Đừng che đậy khoảng trống còn lại bằng prose `NEVER …` ngày càng dài làm phình mỗi lần chạy để đổi lấy an toàn giả; enforcement thật cho phần còn lại thuộc về PreToolUse hook + branch-protection ruleset.
