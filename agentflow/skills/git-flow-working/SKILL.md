...
- **Body phải có:**
  ...
  - Acceptance Criteria của issue mirror thành checklist (tick từng item khi xong — feed vào DoD).
...
| Không bao giờ edit global forbidden paths (`infra/**`, `.github/workflows/**`, `**/*.pem`, `**/.env`) | Áp cho mọi surface. |
| Không bao giờ edit `forbidden` của một surface | Glob no-touch riêng (vd signing config). |
...
Tập no-touch hiệu lực = **hợp** của global forbidden paths và `forbidden` của mọi surface bị chạm (`agentflow.yaml` → `surfaces.<key>.forbidden`; repo không khai báo `surfaces` thì chỉ có global).
