...
1. **Locate the repo config.** Search from the cwd upward for `.claude/agentflow.yaml`.
   ...
   - **Found, but `board.id` empty or `connections.github_project.enabled: false`** → stop: "No AgentFlow board configured here. `/start` is board-driven and needs a board. To enable it, do three things, then re-run `/start`: (1) set `connections.github_project.enabled: true` and a non-empty `board.id` in `.claude/agentflow.yaml` (run `/agentflow-init` and choose *create/link a board*); (2) grant the token the project scope: `gh auth refresh -s project` (add `read:org` for an org board); (3) ensure the 8 board columns exist (init creates them). Or stay labels-only and drive work with `/task` (no board)."
...
3. **Project scope check** (the board is on the decision path now): resolve the board id once via the resolve query in skill: `project-board-protocol`. If it 404s / permission-errors → stop: "`GITHUB_TOKEN` needs `project` scope for board-driven mode — run `gh auth refresh -s project` and retry." (Labels-only installs keep `/task`/agents without it.)
...
1. **Poll** the board for actionable items.
2. For each, **spawn the owning sub-agent** in this repo's context.
...
The canonical `status_map` (skill: `project-board-protocol` → `reference/projects-v2-board.md`) is the only routing table (read it live; nothing is hardcoded). Each board Status maps to a `flow:*` label, an **owner** (`po`/`dev`/`qc`/`human`), and an action. **Actionable = owner is an agent AND not under the soft lock**: that is `Inbox`, `Refined`, `Ready for Dev`, `In QC`, `Changes Requested`. **`In Progress` (owner `dev`) is excluded** — it is under the soft lock, so a card sitting there between polls means DEV paused/blocked and is a **break-out, never a re-spawn**. The `human` owners (`Ready for Human Review`, `Done`) are break-out / terminal too.
...
| `merge #<n>` (only after you reported PR ready)           | Confirm in one line, then `gh pr merge <n> --repo <project.repo>`, then mirror that issue to `Done`. |
...
1. **List board items** with the list query in skill: `project-board-protocol` ("List actionable board items"), paginating. For each item get `{number, statusName, flowLabels, itemId, state}`. Every item belongs to `project.repo`.
2. **Filter to the actionable queue:** keep items where `state == OPEN` **and** `status_map[statusName].owner ∈ {po, dev, qc}` **and** `statusName != "In Progress"`. Exclude `In Progress` here even though its owner is `dev` — the `flow:in-progress` card is under the soft lock and must **never** be re-spawned at poll time (a card paused there is handled as a break-out by step 9, not re-picked). Drop `human`/`Done`/`In Progress`/closed. A **draft** card (no issue number) → cannot route; note it for the user to convert via `/task`.
3. **Order deterministically:** by fixed Status priority `[Changes Requested, In QC, Ready for Dev, Refined, Inbox]` (rework first), then issue number ascending. (`In Progress` is never in the queue — it was filtered out in step 2.)
4. **Take the first actionable item** and process it (serially — the soft lock depends on serial runs). **Skip any item you already broke out to the human this turn** (track them — see step 9). Before spawning, **record the item's current `flow:*` label as `prevLabel`** for the no-progress check in step 9.
5. **Read the live `flow:*` label** for that issue: `gh issue view <n> --repo <project.repo> --json labels,url,title`. **The label is authoritative** — pick the owner from `status_map` by matching `flow_label`, *not* from the board Status. If an actionable item has **no** `flow:*` label yet (a freshly human-added card) → treat it as `Inbox` → owner `po` (PO will set the first label).
...
10. **Safety cap: at most 5 sub-agent calls per user turn.** On reaching it, break and report: "drained 5 of <K> actionable items; reply `go` to continue." (Likely just a busy board, not a bug.)
...
- Never exceed the 5-call cap per user turn. If a loop seems to form, break and report.
