---
description: Khởi động AgentFlow team mode — session trở thành một orchestrator BOARD-DRIVEN, poll GitHub Project board của repo này và chain PMO → DESIGNER → DEV → QC. KHÔNG tạo task (dùng /task hoặc một board card).
---
...
5. In banner (một dòng, parameterized — không hardcode tên):
   ...
   ```
   AgentFlow <project.name> · board <board.number> · notify <ready|off> · ready. New work → /task or a board card; I poll & route PMO → DESIGNER → DEV → QC.
   ```
...
`status_map` canonical (skill: `project-board-protocol` → `reference/projects-v2-board.md`) là routing table duy nhất: mỗi board Status (khớp `board.columns`, map theo `board.columns.<key>`) chỉ tới một **owner** (`pmo`/`designer`/`dev`/`qc`/`human`) và một action. Orchestrator đọc queue và **tin Status** — không có nguồn state thứ hai để đối chiếu.
...
6. **Spawn owning sub-agent** với repo context + board pointer tường minh (`item_id` + Status hiện tại — agent cần chúng để verify qua `get_project_item` và cho compare-then-write; không truyền gì khác; mỗi agent tự đọc `.claude/agentflow.yaml` của repo). **Backstop chống double-pick:** ngay trước spawn, re-read Status qua `get_project_item` — nếu Status không còn là state bạn vừa route (vd một terminal khác đã đẩy ticket sang "In Progress") → KHÔNG spawn, coi Status vừa đọc là `newStatus` và nhảy tới step 8.
   ...
   - DESIGNER: `Agent(subagent_type="designer", prompt="ISSUE: #<n>\nREPO: <project.repo>\nITEM_ID: <item_id>\nSTATUS: <status>")`
...
9. **Safety cap: tối đa 12 sub-agent call mỗi user turn** (một ticket không-UI đầy đủ gồm một vòng rework là PMO+DEV+QC+DEV+QC = 5; một ticket UI có bật design review, với một QC rework và một design rework, là PMO+DESIGNER+DEV+QC+DEV+QC+DESIGNER+DEV+QC+DESIGNER = 10; 12 chừa headroom). Khi chạm cap, break và báo: "drained N items; reply `go` để tiếp tục **các ticket inbox còn lại**."
...
- Không bao giờ vượt cap 12 call mỗi user turn. Nếu có vẻ hình thành một loop, break và báo.
